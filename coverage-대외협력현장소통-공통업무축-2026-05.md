# Coverage — 대외협력·현장소통 공통업무축 2026-05

## 목적
대외협력·현장소통 관련 hub, 첫주 path, FAQ, source, process, checklist note를 한곳에 묶어 전입자가 협력 대상 확인부터 후속 조치 검토까지 다시 찾게 한다.

## 핵심 진입로
- [[hub-대외협력현장소통]]
- [[newcomer-path-대외협력현장소통-첫주-2026-05]]

## 먼저 읽을 FAQ
- [[faq-현장의견과-공식입장은-어떻게-구분하나]]
- [[faq-대외협력현장소통-현장의견-표현예시]] — 현장 의견을 “공식 입장처럼” 쓰지 않도록 표현 단위의 안전 문장을 고른다.

## 자료·초안 흐름
- [[source-대외협력현장소통-자료대조순서]] — 공개자료, 담당 확인자료, 현장 의견, 내부검토자료를 먼저 분리한다.
- [[process-대외협력현장소통-결과초안-작성흐름]] — 결과초안에서 확정 사실, 현장 의견, 후속조치 후보, 담당 확인 필요사항을 나눠 쓴다.
- [[checklist-대외협력현장소통-후속조치-검토]] — 담당 부서·기한·보고경로·공개경계를 제출 전 확인한다.
- [[checklist-대외협력현장소통-민감정보-masking-제출전검토]] — 개인정보·민원인·업체·관계자 식별 위험을 가린다.
- [[checklist-대외협력현장소통-국회감사민원-전환검토]] — 현장소통 결과가 국회·감사·민원 답변으로 바뀔 때 재사용 금지선을 다시 탄다.

## 연결 업무축
- [[hub-협업회의운영]]
- [[hub-정보공개기록관리]]
- [[hub-민원대응]]
- [[hub-홍보대응]]
- [[hub-장관보고]]
- [[hub-국회대응]]

## 전입자 판단 순서
1. 협력 대상과 소통 목적을 확인한다.
2. 현장 의견과 공식 입장을 나눈다.
3. 공유 가능한 자료와 내부 검토자료를 분리한다.
4. 후속 조치별 담당 부서, 기한, 보고 경로를 표시한다.
5. 기록물·정보공개 가능성과 대외 설명 가능성을 함께 확인한다.

## 공통 masking FAQ
- [[faq-공통업무축-민감정보-masking-사례구분]]
- [[checklist-대외협력현장소통-민감정보-masking-제출전검토]]
- [[checklist-대외협력현장소통-국회감사민원-전환검토]]
- [[manifest-newcomer-journey-대외협력현장소통-국회감사민원전환-2026-05]]

## 공개/비공개 decision points
1. **공개 가능**: 이미 공개된 행사 개요, 확정된 기관 입장, 공개자료로 확인되는 통계·일정·제도 설명.
2. **담당 확인 필요**: 현장 의견 요약, 후속조치 후보, 관계기관 협의 상태, 부서 간 역할 분담.
3. **비공개 유지**: 개인정보, 민원인 식별정보, 업체별 민감정보, 비공개 내부검토 의견, 미확정 정책안, 현장 관계자 개인 평가나 발언 왜곡.
4. **전환 시 재검토**: 국회 자료요구·감사 확인·민원 답변·홍보 문안으로 바뀌면 [[manifest-newcomer-journey-대외협력현장소통-국회감사민원전환-2026-05]]와 대상 업무축 checklist를 다시 탄다.

## Walkthrough exposure
- 전용 walkthrough note는 아직 만들지 않았다. 현재는 [[section-대외협력현장소통-전환묶음-2026-05]]와 전환 manifest가 FAQ/source/process/checklist 순서를 안내하며, 새 walkthrough를 만들기 전에는 placeholder wikilink를 두지 않는다.

## Manifest
- [[manifest-newcomer-journey-대외협력현장소통-2026-05]]
- [[quality-gate-대외협력현장소통-2026-05]]

## 다음 빈칸
1. 대외협력·현장소통 manifest와 quality gate는 [[manifest-newcomer-journey-대외협력현장소통-2026-05]] / [[quality-gate-대외협력현장소통-2026-05]]로 닫혔다.
2. 협업·회의운영과 대외협력·현장소통의 후속조치 경계 process는 [[process-협업회의운영-대외협력현장소통-후속조치경계]]로 닫혔다.
3. 후속조치 제출전 checklist 분화는 [[checklist-대외협력현장소통-민감정보-masking-제출전검토]]로 시작했다.
4. 현장의견/공식입장 표현 예시 FAQ는 [[faq-대외협력현장소통-현장의견-표현예시]]로 시작했다.
5. 국회·감사·민원 전환 흐름은 [[manifest-newcomer-journey-대외협력현장소통-국회감사민원전환-2026-05]]로 manifest까지 닫혔다.
6. 대외협력·현장소통 산출물 묶음 분리는 [[section-대외협력현장소통-전환묶음-2026-05]]로 시작했고, README/landing/docs 노출까지 완료됐다.
7. 다음은 이 section을 국회·감사·민원 coverage의 교차 진입부에도 노출할지 검토한다.

## Section
- [[section-대외협력현장소통-전환묶음-2026-05]]
- [[section-공통업무축-내부운영협업대외소통-2026-05]]
