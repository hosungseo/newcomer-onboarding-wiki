# Section — 최근 boundary note 묶음 2026-05

## 목적
최근 boundary FAQ/checklist를 하나의 section으로 묶어 공통업무축 전입자가 공개/비공개 경계를 빠르게 찾게 한다.

## 진입 순서
- [[manifest-newcomer-journey-boundary-notes-2026-05]] — 질문 유형별 route.
- [[index-최근-boundary-note-묶음-2026-05]] — 업무축별 entry map.
- [[coverage-최근-boundary-note-묶음-2026-05]] — 노출·연결 상태.
- [[manifest-institution-to-boundary-router-2026-05]] — 기관군 전입 경로에서 boundary note로 되돌아가는 router.
- [[quality-gate-최근-boundary-note-묶음-2026-05]] — 공개/비공개 안전 점검.

## Boundary note 묶음
- [[faq-계약자료-공개자료와-업체민감정보는-어떻게-나누나]]
- [[faq-조직정원-공식조직도와-업무분장은-어떻게-나누나]]
- [[checklist-예산대응-단계별분기-제출전검토]]
- [[checklist-민원대응-민감정보-masking-제출전검토]]
- [[checklist-홍보대응-대외전환-문안분기-제출전검토]]
- [[faq-법령설명-법률자문성결론은-어디서-멈추나]]

## 사용 순서
1. 상황이 명확하면 journey manifest에서 바로 route를 고른다.
2. 업무축만 알면 index에서 해당 FAQ/checklist를 고른다.
3. 공개/비공개 멈춤선이 충분한지 quality gate로 확인한다.
4. 새 note가 추가되면 coverage와 index에 함께 반영한다.

## 금지선
개인정보, 비공개 내부검토, 미확정 정책안, 업체별 민감정보, 법률자문성 결론, 감사·수사·보안 세부는 포함하지 않는다.
