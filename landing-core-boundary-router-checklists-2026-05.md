# Landing — Core Boundary Router Checklists 2026-05

## 목적
전입자가 핵심 업무축에서 공개/비공개 경계를 먼저 확인해야 할 때 바로 고르는 landing이다.

## 바로 고르기
- 계약조달 자료·업체정보·제안서 → [[checklist-계약조달-boundary-router-질문형-검토]]
- 조직정원 조직도·정원표·업무분장 → [[checklist-조직정원-boundary-router-질문형-검토]]
- 예산요구·국회심의·산출근거 → [[checklist-예산대응-boundary-router-질문형-검토]]
- 민원 원문·답변 초안·반복민원 → [[checklist-민원대응-boundary-router-질문형-검토]]
- 보도자료·브리핑·대외 설명 → [[checklist-홍보대응-boundary-router-질문형-검토]]
- 법령 설명·제도 안내·해석 요청 → [[checklist-법령대응-boundary-router-질문형-검토]]

## 묶음과 검증
- [[coverage-core-boundary-router-landing-2026-05]] — 핵심 업무축 boundary checklist landing 노출 coverage.
- [[manifest-core-boundary-router-checklists-2026-05]]
- [[quality-gate-core-boundary-router-checklists-2026-05]]
- [[release-candidate-core-boundary-checklists-2026-05]] — 핵심 업무축 boundary checklist 묶음 release candidate.
- [[coverage-institution-boundary-core-hubs-2026-05]]

## 상위 경로
- [[walkthrough-institution-boundary-router-question-flow-2026-05]]
- [[manifest-institution-to-boundary-router-2026-05]]
- [[checkpoint-2026-05-05-boundary-router-ecosystem]]

## 금지선
개인정보, 비공개 내부검토, 미확정 정책안, 인사·감사·징계·수사·보안·업체 민감정보, 법률자문성 결론은 포함하지 않는다.
