# Manifest — Core Boundary Router Checklists 2026-05

## 목적
- [[quality-gate-core-boundary-router-checklists-2026-05]] — 핵심 업무축 boundary router checklist 묶음 품질 점검.
- [[landing-core-boundary-router-checklists-2026-05]] — 핵심 업무축 boundary checklist를 바로 고르는 landing.
계약조달·조직정원·예산대응·민원대응·홍보대응·법령대응 hub에서 [[manifest-institution-to-boundary-router-2026-05]]로 돌아가기 전 사용하는 질문형 checklist 묶음을 고정한다.

## Checklist bundle
- [[checklist-계약조달-boundary-router-질문형-검토]]
- [[checklist-조직정원-boundary-router-질문형-검토]]
- [[checklist-예산대응-boundary-router-질문형-검토]]
- [[checklist-민원대응-boundary-router-질문형-검토]]
- [[checklist-홍보대응-boundary-router-질문형-검토]]
- [[checklist-법령대응-boundary-router-질문형-검토]]

## Route
1. 기관 또는 core 업무 hub에서 시작한다.
2. 해당 checklist로 공개/비공개 경계 질문을 확인한다.
3. 필요하면 [[walkthrough-institution-boundary-router-question-flow-2026-05]]로 기관 route를 다시 탄다.
4. 최종적으로 [[manifest-institution-to-boundary-router-2026-05]] 또는 관련 boundary note로 회귀한다.

## 상위 상태판
- [[coverage-institution-boundary-core-hubs-2026-05]]
- [[quality-gate-institution-boundary-hub-expansion-2026-05]]
- [[checkpoint-2026-05-05-boundary-router-ecosystem]]

## 금지선
개인정보, 비공개 내부검토, 미확정 정책안, 인사·감사·징계·수사·보안·업체 민감정보, 법률자문성 결론은 포함하지 않는다.
